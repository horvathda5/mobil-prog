                ...
                FAuth.sendPasswordResetEmail(forgetpassword.getEditText().getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            mDialog.dismiss();
                            ReusableCodeForAll.ShowAlert(ChefForgotPassword.this, "", "A jelszót elküldtük az emailt címedre!");
                        } else {
                            mDialog.dismiss();
                            ReusableCodeForAll.ShowAlert(ChefForgotPassword.this, "Hiba!", task.getException().getMessage());
                        }
                    }
                });
            }
